...
def train(data_loader, model, optimizer, scheduler, criterion, device, save_checkpoints, checkpoint_path_prefix, current_epoch): # val_acc removed
    model.train()
    total_loss = 0
    correct = 0
    total = 0
    for data in tqdm(data_loader, desc="Iterating training graphs", unit="batch"):
        ...
        # Scheduler step: Only for per-batch schedulers.
        # ReduceLROnPlateau is handled at the epoch level in the main loop.
...
def main(args):
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay) # Default values from the notebook
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=args.scheduler_patience, verbose=True)
    ...
    if args.train_path:
        ...
        for epoch in range(num_epochs):
            ...
            # CORRECTED call to train function (val_acc removed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=150, help='Number of epochs to train (default: 200).') # Default was 25, changed to 200 as in notebook
    ...
    parser.add_argument('--gnn', type=str, default='gcn', choices=['gin', 'gin-virtual', 'gcn', 'gcn-virtual'], help='GNN architecture (default: gin).')
    parser.add_argument('--num_layer', type=int, default=4, help='Number of GNN message passing layers (default: 5).')
    parser.add_argument('--emb_dim', type=int, default=150, help='Dimensionality of hidden units (default: 300).') # Default was 300, common examples use 100 or 64
    ...
    parser.add_argument('--baseline_mode', type=int, default=1, choices=[1, 2], help="Loss mode: 1 (CrossEntropy), 2 (GeneralizedCrossEntropy) (default: 2).") # Removed 3 if FC is commented out
    parser.add_argument('--loss_param', type=float, default=0.5, help="Parameter for GCE (q value) (default: 0.7).")
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-5, help='Weight decay (L2 penalty) (default: 1e-5).')
    parser.add_argument('--scheduler_patience', type=int, default=10, help='Patience for ReduceLROnPlateau scheduler (default: 10).')
